...
while [ true ]; do
  while [ "$time" = "$(date | awk '{ print $4, $5 }')" ]; do
    if [ $bl_level -ne $(cat /sys/class/backlight/nvidia_0/brightness) ]; then
      break
    elif [ $volume_lvl -ne $(pamixer --get-volume) ]; then
      break
    fi
    if [ "$volume_mute" = "$(pamixer --get-mute)" ]; then
    else
      break
    fi
    sleep 0.1
  ...
  if [ $bl_level -le 16 -a $bl_level -ge 0 ]; then
    ...
    bl_ico="󰃛 "
  elif [ $bl_level -le 44 -a $bl_level -gt 30 ]; then
    bl_ico="󰃚 "
  elif [ $bl_level -le 58 -a $bl_level -gt 44 ]; then
    bl_ico="󰃝 "
  elif [ $bl_level -le 72 -a $bl_level -gt 58 ]; then
    bl_ico="󰃞 "
  elif [ $bl_level -le 86 -a $bl_level -gt 72 ]; then
    bl_ico="󰃟 "
  elif [ $bl_level -le 100 -a $bl_level -gt 86 ]; then
    bl_ico="󰃠 "
  fi
  bat_level=$(cat /sys/class/power_supply/BAT0/capacity)
